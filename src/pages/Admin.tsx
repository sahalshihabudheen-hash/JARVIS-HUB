...
import { CopyIcon as Copyright, Image as ImageIcon, Save, Activity as ActivityIcon, Type, FileText } from "lucide-react";
...
const Admin = () => {
  ...
  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      // In a real app, this would upload to auth/storage. For this demo, we create a local blobl URL
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans">
      ...
      <main className="container max-w-5xl mx-auto pt-28 pb-20 px-4">
        ...
        <div className="bg-[#111111] border border-white/5 rounded-2xl p-6 md:p-8 shadow-2xl">
          ...
          <div className="space-y-2 mt-6">
            {activityLog.map((activity, index) => {
              ...
              return (
                <div key={activity.id} className="flex items-center justify-between p-4 rounded-xl hover:bg-white-[0.02] transition-colors border border-transparent hover:border-white/5">
                  <div className="flex items-center gap-4">
                    <img 
                      src={activity.mediaPoster} 
                      alt={activity.mediaTitle} 
                      className="w-12 h-12 object-cover rounded-md bg-[#222]"
                    />
                    <div>
                      <h4 className="text-sm font-medium text-white/90">{activity.mediaTitle}</h4>
                      <p className="text-xs text-white/40 mt-0.5 capitalize">{activity.mediaType} stream</p>
                    </div>
                  </div>
                  <div className="text-right flex flex-col items-end">
                    <span className="text-sm font-medium text-[#ffd700]">{activity.userEmail}</span>
                    <span className="text-xs text-white/30 mt-0.5">{timeDisplay}</span>
                  </div>
                </div>
              );
            })}
