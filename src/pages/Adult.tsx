    ...
    const loadData = async () => {
      try {
        if (user?.uid) {
          ...
          setAdultHistory(Array.isArray(syncedHistory) ? syncedHistory : []);
        } else {
          const history = getAdultHistory();
          setAdultHistory(Array.isArray(history) ? history : []);
        }
      } catch (e) {
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-md animate-fade-in p-4">
          <div className="max-w-xl w-full bg-card border border-white/10 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 shadow-3xl text-center space-y-8 max-h-[95vh] overflow-y-auto">
            ...
            <div className="flex flex-wrap justify-center gap-3">
                {Array.isArray(onboardingGenres) && onboardingGenres.map(genre => (
                 <button
                   key={genre}
                   onClick={() => togglePreferredGenre(genre)}
                   className={cn(
                     "px-5 py-2.5 rounded-full text-xs font-bold transition-all border",
                     Array.isArray(preferredGenres) && preferredGenres.includes(genre)
